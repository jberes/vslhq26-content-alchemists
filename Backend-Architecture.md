...
`/auth` (register, login, refresh, logout, change-password) · `/me` (profile, avatar) · `/settings` · `/campaigns` · `/campaigns/{id}/artifacts` (ETag CRUD) · `/campaigns/{id}/artifacts/{id}/revisions` (list, get, restore — B9) · `/campaigns/{id}/image-slots` (list, patch, clear — B9) · `/assets` · `/brands` · `/blob` (sas, test, list) · `/images/composite` (headline overlay — B9) · `/media` (clip-jobs, frames — B9) · `/publish` (channels, posts, test) · `/schedule` (week, create, move, cancel — B9) · `/seo` (analyze, keyword-plan, report, share) · `/ai` (status probe, campaign fan-out, per-generator endpoints, render-images, transcription) · `/health`
