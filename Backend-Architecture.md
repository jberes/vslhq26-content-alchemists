...
| **Image plan** (`Services/Images`) | Typed `ImageSlot` rows per campaign (kind, target dimensions, prompt, model alias, source segment, state, published URL) reserved when a run starts; slot-accurate resize/crop from the model's native size; server-side headline **overlay compositor** with safe-area inset | Slot state is the single source for every image counter in the UI (ADR-012, ADR-013) |
...
`/auth` (register, login, refresh, logout, change-password) · `/me` (profile, avatar) · `/settings` · `/campaigns` (incl. `/{id}/seo-targets` GET/PUT — ADR-023) · `/campaigns/{id}/artifacts` (ETag CRUD) · `/campaigns/{id}/artifacts/{id}/revisions` (list, get, restore — B9) · `/campaigns/{id}/image-slots` (list, patch, clear — B9) · `/assets` · `/brands` (incl. `/lookup` draft-from-URL with SSRF guard, brand assets + per-kind templates) · `/blob` (sas, test, list, `assets/{id}/content` upload proxy — frontend ADR-F26) · `/images/composite` (headline overlay — B9) · `/media` (clip-jobs, frames — B9) · `/publish` (channels, posts, test) · `/schedule` (week, create, move, cancel — B9) · `/seo` (analyze, keyword-plan, **research** — ADR-023, report, share) · `/ai` (status probe, campaign fan-out, per-generator endpoints, render-images, transcription, `runs/latest` — ADR-022) · `/health`
