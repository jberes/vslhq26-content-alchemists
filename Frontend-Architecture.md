...
- *MAUI packaging notes:* Whisper.net.Runtime's Catalyst static libs only wire into DIRECT package referencers, so the MAUI head references the runtime package itself; NuGet's generic runtime-asset copy also had to be pruned of non-Apple natives (linux `.so` broke the Catalyst bundler).
...
- **SEO / AEO desk** as the fourth campaign view: scorecard column (score, one-line verdict, titles / JSON-LD / image-alt rows), keyword table + share-of-voice bars on Blazor-ApexCharts (ADR-F24), content angles with "Draft these as artifacts"; the A/B YouTube titles from the keyword plan; **projected-vs-actual honesty rule stated on the surface** for unpublished campaigns. Share-link UX; blog metadata tabs.
...
### Phase F10 — Image Studio *(size L · needs B9.1–B9.5)*
